...
const parseSetEnumTypeDef = function (type, prefixLen) {
  ...
  return type
    ...
    .split(',')
    .map(function (opt) {
      return opt[0] === '"' || opt[0] === "'" ? opt.substr(1, opt.length - 2) : opt;
    });
};
...
const parseNewDecimal = function (parser, column) {
  ...
  // Grab buffer portion
  const size =
    uncompIntegral * 4 + compressedBytes[compIntegral] + uncompFractional * 4 + compressedBytes[compFractional];
  ...
  switch (column.type) {
    ...
    case MysqlTypes.STRING: {
      ...
      if (defPrefix === 'binary') {
        ...
        result = new Uint8Array(parser.parseBuffer(parser.parseBuffer(size).copy(result)));
    ...
    case MysqlTypes.BIT:
      ...
      result = new Uint8Array(parser._buffer.slice(parser._offset, parser._offset + size));
      parser._offset += size; // Move binlog parser position forward
      break;
