...
function defineTypeTest(name, fields, testRows, customTest) {
  ...
  const testQueries = [`CREATE TABLE ${TEST_TABLE} (${fieldText})`, 'SET @@session.time_zone = "+00:00"']
    .concat(
      testRows.map(
    ...
    .concat(['SET @@session.time_zone = "SYSTEM"', `SELECT * FROM ${TEST_TABLE}`]);
...
defineTypeTest(
  'geometry',
  ['GEOMETRY'],
  [["GeomFromText('POINT(1 1)')"], ["GeomFromText('POLYGON((0 0,10 0,10 10,0 10,0 0),(5 5,7 5,7 7,5 7, 5 5))')"]]
);
...
testDb.requireVersion('5.6.4', () => {
  defineTypeTest(
    'time_fraction',
    ['TIME(0) NULL', 'TIME(1) NULL', 'TIME(3) NULL', 'TIME(6) NULL'],
    [
  ...
  defineTypeTest(
    'datetime_fraction',
    ['DATETIME(0) NULL', 'DATETIME(1) NULL', 'DATETIME(4) NULL', 'DATETIME(6) NULL'],
    [
      [
        "'1000-01-01 00:00:00'",
        "'1000-01-01 00:00:00.5'",
        "'1000-01-01 00:00:00.9999'",
        "'1000-01-01 00:00:00.123456'"
      ],
      [
        "'9999-12-31 23:59:59'",
        "'9999-12-31 23:59:59.9'",
        "'9999-12-31 23:59:59.6543'",
        "'9999-12-31 23:59:59.000001'"
      ],
      ["'9999-12-31 23:59:59'", "'9999-12-31 23:59:59.1'", "'9999-12-31 23:59:59.1234'", "'9999-12-31 23:59:59.4326'"],
      ["'2014-12-27 01:07:08'", "'2014-12-27 01:07:08.0'", "'2014-12-27 01:07:08.0001'", "'2014-12-27 01:07:08.05'"]
    ]
  );
  ...
  defineTypeTest(
    ...
    ['TIMESTAMP(3) NULL'],
    [["'1970-01-01 00:00:01.123'"], ["'2038-01-18 03:14:07.900'"], ["'2014-12-27 01:07:08.001'"]]
  );

  defineTypeTest(
    'datetime_then_decimal',
    ['DATETIME(3) NULL', 'DECIMAL(30, 10) NULL'],
    [
      ["'1000-01-01 00:00:00.123'", 10.1],
...
testDb.requireVersion('5.7.8', () => {
  defineTypeTest(
    ...
    function (test, event) {
      // caution here , don't use arrow function
